                ...
                @foreach($users as $user)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                            @if($user->image_path)
                                <img class="w-10 h-10 rounded-full" src="{{ asset('storage/'.$user->image_path) }}" alt="Jese image">
                            @else 
                        ...
                        <td class="px-6 py-4">
                            <div class="filter">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                                    ...
                                    <li><a data-modal-target="targets-modal" data-user="{{$user->name}}" data-id="{{ $user->id }}" data-modal-toggle="targets-modal" style="cursor: pointer" class="dropdown-item targets_modal_toggle"><i class="bi bi-bullseye"></i>Set Target</a></li>
...
<script>
    $(document).on("click", ".targets_modal_toggle", function () {
        const UserID = $(this).data('id');
        const userData = $(this).data('user');
        $(".modal-bod #user_id").val( UserID );
        $(".modal-bod h3").text("Target for " + userData);

    });
</script>
<div id="targets-modal" tabindex="-1" aria-hidden="true" class="modal-bod fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            ...
            <div class="px-6 py-6 lg:px-8">
                <form class="space-y-6" action="{{route('targets.store')}}" method="post">
                    <div class="d-flex justify-content-center bg-[#012970] align-items-center">
                        <h3 style="color: white;padding-top: 7px;font-size: 22px;font-weight: bold;">Target</h3>
                    </div>
                    @csrf
